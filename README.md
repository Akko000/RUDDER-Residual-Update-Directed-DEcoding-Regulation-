...
α_t = softplus(k s_t + c)
β_t = softplus(-k s_t + c)
g_t = α_t / (α_t + β_t)

The final steering vector is:

v_t^steer = α_max · g_t · v_CARD
...
├── rudder/                 # Core RUDDER implementation
│   ├── card.py             # CARD extraction
│   ├── beta_gate.py        # Adaptive Beta Gate
│   └── hooks.py            # Residual-stream hooks
